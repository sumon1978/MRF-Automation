...
def load_edit_rules(edit_rule_file):
    ...
    for _, row in df_rules.iterrows():
        ...
        flag = norm(row.iloc[0])

        if flag != "Y":
            continue

        old_mat = norm(row.iloc[1])

        if old_mat:
            rules[old_mat] = {
                "new_mat": row.iloc[2],
                "desc": row.iloc[7] if len(row) > 7 else ""
            }
        ...
        for ws in wb.worksheets:
            for r in range(1, ws.max_row + 1):
                ...
                if material in rules:
                    ws[f"C{r}"] = rules[material]["new_mat"]
                    ws[f"G{r}"] = rules[material]["desc"]

        output_path = os.path.join(
            temp_dir,
            os.path.splitext(uploaded.name)[0] + "_EDITED.xlsx"
        )
